...
export class Minimap {
  ...
  update(player: Player, remotes: Remotes, settings: Settings, myVoice: number): void {
    // This is a map of the island, so it's no use in the shadow realm — and
    // `place()` clamps to the edge, which would park you on a shoreline you
    // aren't anywhere near.
    const away = inRealm(player.group.position.x, player.group.position.z)
    const want = settings.minimap && !away
    ...
    for (const { id, pos, color, talk } of remotes.blips()) {
      if (inRealm(pos.x, pos.z)) continue // they're through the portal, not off the beach
