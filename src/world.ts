...
//
// Exported because the map draws the world from it (map.ts): it's the same
// surface heightAt reports, minus the crater walk, which is invisible at map
// scale and would mean 20k crater-list scans per redraw.
...
export function addCraters(list: Crater[]): DestroyedProp[] {
  ...
  // Every island's mesh, since a crater can land on any of them.
  for (const terrainGeo of terrainGeos) {
