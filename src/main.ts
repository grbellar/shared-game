...
const SUN_AIM = new THREE.Vector3() // scratch for the shoot-the-sun aim check
...
function attack(): void {
  ...
  if (weapon === 'gun' && now - lastAttack > 800) {
    ...
    if (daynight.sunUp && dir.dot(daynight.sunDirection(SUN_AIM)) > SUN_AIM_DOT) {
...
window.addEventListener('keydown', (e) => {
  ...
  // Held, FPS-style. N rather than Tab, which is the map.
  if (e.code === 'KeyN' && !chat.isOpen) {
